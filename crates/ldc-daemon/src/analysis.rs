...
pub fn local_analysis(
    ...
    TechnicalAnalysis {
        ...
        insights: vec![format!(
            "O dia em {project} teve {events} eventos, {commits} commit(s) e foco inicial em {first_file}.",
...
fn build_prompt(summary: &DailySummary) -> String {
    format!(
        "Voce e um tech lead analisando o dia de desenvolvimento para gerar contexto real de um post LinkedIn. Retorne SOMENTE JSON valido com os campos insights (array de strings), tech_stack (array de strings), complexity (0-10), learnings (array de strings). Resumo do dia: {}",
